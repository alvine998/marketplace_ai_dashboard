        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {customers.map((customer) => (
                    <div key={customer.id} className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm hover:border-slate-300 hover:shadow-xl hover:shadow-blue-500/5 transition-all group relative">
                        <button className="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-900 transition-colors">
                            <MoreHorizontal className="w-5 h-5" />
                        </button>

                        <div className="flex items-start gap-4 mb-6">
                            <div className="w-16 h-16 rounded-2xl bg-slate-50 border border-slate-200 overflow-hidden shrink-0">
                                <img src={customer.avatar} alt={customer.name} className="w-full h-full object-cover" />
                            </div>
                            <div className="min-w-0">
                                <h3 className="font-bold text-lg text-slate-900 truncate">{customer.name}</h3>
                                <div className="flex items-center gap-1.5 mt-1">
                                    <span className={`w-2 h-2 rounded-full ${customer.status === 'Active' ? 'bg-green-500' :
                                        customer.status === 'Pending' ? 'bg-amber-500' :
                                            'bg-slate-400'
                                        }`} />
                                    <span className="text-xs font-medium text-slate-500 uppercase tracking-tighter">{customer.status}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="pt-5 border-t border-slate-100 flex items-center justify-between">
                            ...
                            <button className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-blue-600 hover:bg-blue-600 hover:text-white transition-all">
                ...
                <button className="bg-white border-2 border-dashed border-slate-200 p-6 rounded-3xl hover:border-blue-500/50 hover:bg-blue-50 transition-all group flex flex-col items-center justify-center min-h-[300px]">
                    <div className="w-14 h-14 rounded-full bg-slate-50 flex items-center justify-center mb-4 group-hover:bg-blue-500/10 group-hover:scale-110 transition-all text-slate-400 group-hover:text-blue-500">
