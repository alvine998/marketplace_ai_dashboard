...
interface AdminUser {
    ...
    name: string;
    email: string;
    role: 'Super Admin' | 'Editor' | 'Penampil';
...
const AdminManagementPage: React.FC = () => {
    ...
    const [admins] = useState<AdminUser[]>([
        { id: '1', name: 'Alvin Yoga', email: 'alvin@marketai.com', role: 'Super Admin', status: 'active', lastActive: 'sekarang' },
        { id: '2', name: 'Sarah Miller', email: 'sarah@marketai.com', role: 'Editor', status: 'active', lastActive: '2jam lalu' },
        { id: '3', name: 'John Doe', email: 'john@marketai.com', role: 'Penampil', status: 'invited', lastActive: 'N/A' },
        { id: '4', name: 'Michael Chen', email: 'chen@marketai.com', role: 'Editor', status: 'active', lastActive: 'Kemarin' },
    ]);
    ...
    const rolePermissions = {
        'Super Admin': [
            ...
            'Kelola pengguna admin lainnya',
            'Visibilitas data keuangan',
            'Akses konfigurasi API'
        ],
        'Editor': [
            'Kelola katalog produk',
            'Tangani persetujuan penjual',
            'Siarkan notifikasi',
            'Balas tiket pelanggan'
        ],
        'Penampil': [
            'Metrik dasbor baca-saja',
            'Lihat daftar produk',
            'Pantau log sistem',
            'Lacak pesanan terbaru'
        ]
    };
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="text-left">
                    <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Manajemen Admin</h1>
                    <p className="text-slate-500 mt-1 font-medium">Kontrol izin akses dan peran pengguna dasbor.</p>
                </div>
                <button
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-slate-50 text-slate-500 text-[10px] font-bold uppercase tracking-[0.2em] border-b border-slate-200">
                                <th className="px-8 py-5">Akun Pengguna</th>
                                <th className="px-8 py-5">Peran Otentikasi</th>
                                <th className="px-8 py-5">Status Aktivitas</th>
                                <th className="px-8 py-5">Log Terakhir</th>
                                <th className="px-8 py-5 text-right">Akses</th>
                            </tr>
                        </thead>
                                ...
                                <tr key={user.id} className="group hover:bg-slate-50 transition-all border-b border-slate-100 last:border-none">
                                    ...
                                    <td className="px-8 py-6 text-right">
                                        <div className="flex items-center justify-end gap-2 opactiy-60 group-hover:opacity-100 transition-opacity">
                                            <button className="p-2 bg-white hover:bg-slate-50 text-slate-400 hover:text-blue-600 rounded-lg transition-all border border-slate-200 shadow-sm" title="Kelola Izin">
                                                <Key className="w-4 h-4" />
                                            </button>
                                            <button className="p-2 bg-slate-50 hover:bg-slate-100 text-slate-500 hover:text-slate-900 rounded-lg transition-all border border-slate-200 shadow-sm" title="Ubah Rekan">
                                                <Edit3 className="w-4 h-4" />
                                            </button>
                                            <button className="p-2 bg-slate-50 hover:bg-red-50 text-slate-500 hover:text-red-500 rounded-lg transition-all border border-slate-200 shadow-sm" title="Matikan Akses">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-lg bg-white border border-slate-200 rounded-[2.5rem] shadow-2xl p-8 animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 text-left">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                <h2 className="text-2xl font-bold text-slate-900">Undang Rekan</h2>
                                <p className="text-slate-500 text-sm font-medium mt-1">Berikan akses administratif ke dasbor Anda</p>
                            </div>
                        ...
                        <div className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest pl-1">Nama Lengkap</label>
                                <input
                                    type="text"
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest pl-1">Peran Dasbor</label>
                                <div className="grid grid-cols-3 gap-3">
                                    {(['Super Admin', 'Editor', 'Penampil'] as AdminUser['role'][]).map((role) => (
                                        <button
                                            key={role}
                                        ...
                                        >
                                            ...
                                            {role === 'Penampil' && <User className={`w-5 h-5 ${newAdmin.role === role ? 'text-white' : 'text-slate-400 group-hover:text-indigo-500'}`} />}
                            ...
                            <div className="bg-slate-50 border border-slate-100 rounded-3xl p-5">
                                <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.15em] mb-4 flex items-center gap-2">
                                    <Key className="w-3.5 h-3.5 text-indigo-500" /> Kapasitas Berizin
                                </h4>
