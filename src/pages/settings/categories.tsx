        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div className="lg:col-span-2 bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
                    <div className="p-6 border-b border-slate-200 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4">
                            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Total Categories: 12</p>
                        </div>
                    </div>
                                ...
                                <div className={`group flex items-center justify-between p-4 rounded-2xl border transition-all ${expanded.includes(cat.id) ? 'bg-slate-50 border-slate-200' : 'bg-white border-slate-100 hover:border-slate-200'}`}>
                                    <div className="flex items-center gap-4">
                                        <button
                                            ...
                                            className={`p-1.5 rounded-lg transition-all ${expanded.includes(cat.id) ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'hover:bg-slate-100 text-slate-400'}`}
                                        >
                                            ...
                                            <div key={sub.id} className="flex items-center justify-between p-3.5 bg-white shadow-sm border border-slate-100 rounded-xl hover:border-slate-200 transition-all group">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="text-xs text-left">
                                                        ...
                                                        <p className="text-slate-400 font-medium">{sub.productCount} Items</p>
                                                    </div>
                                                </div>
                                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <button className="p-1.5 hover:bg-slate-100 text-slate-400 hover:text-slate-900 rounded-lg transition-all"><Edit className="w-3.5 h-3.5" /></button>
                                                    <button className="p-1.5 hover:bg-red-50 text-slate-400 hover:text-red-600 rounded-lg transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                ...
                <div className="bg-white border border-slate-200 p-8 rounded-3xl shadow-sm text-left">
                    <div className="mb-8">
                        <h3 className="text-lg font-bold text-slate-900">Quick Add Entry</h3>
                        <p className="text-xs text-slate-500 mt-1 font-medium italic">Create a new classification level</p>
                    </div>

                    <div className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Entry Name</label>
                            <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all font-['Inter'] placeholder-slate-400" placeholder="e.g. Gaming Gear" />
                        </div>

                        <div className="space-y-2">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Parent Level</label>
                            <select className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all appearance-none cursor-pointer">
                                <option value="none" className="bg-white">Create as Root</option>
                        ...
                        <div className="space-y-2 pt-2">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Assign Icon</label>
                            <div className="grid grid-cols-4 gap-2">
                                {[Box, Smartphone, Laptop, Clock, Folder, Layers, ImageIcon, Plus].map((Icon, i) => (
                                    <button key={i} className={`p-3 rounded-xl border transition-all flex items-center justify-center ${i === 2 ? 'border-blue-500 text-blue-600 bg-blue-50' : 'border-slate-200 bg-slate-50 text-slate-400 hover:border-blue-500 hover:text-blue-500 hover:bg-blue-50'}`}>
                        ...
                        <button className="w-full py-4 bg-slate-100 hover:bg-blue-600 hover:text-white text-slate-600 font-bold rounded-2xl transition-all shadow-sm active:scale-95 text-sm mt-4">
